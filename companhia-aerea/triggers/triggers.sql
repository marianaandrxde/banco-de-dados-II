 ...
 */
 ...
 RETURNS TRIGGER AS $$
 DECLARE DATA_VOO_MINIMA DATE;
 BEGIN
    SELECT MIN(DATA_VOO) INTO DATA_VOO_MINIMA FROM AVIAO A 
    JOIN ITEM_RESERVA IR
    JOIN RESERVA R WHERE
    WHERE IR.ID_RESERVA = NEW.ID_RESERVA 
    AND IR.ID_RESERVA = R.ID_RESERVA;
...
CREATE TRIGGER T_BLOQUEIO_CANCELAMENTO_RESERVA() BEFORE DELETE ON RESERVA FOR EACH ROW EXECUTE PROCEDURE T_BLOQUEIO_CANCELAMENTO_RESERVA();
 ...
 */
 ...
 BEGIN
    ...
    JOIN ITEM_RESERVA IR
    JOIN RESERVA R WHERE
    WHERE IR.ITEM_RESERVA = NEW.ITEM_RESERVA;
...
CREATE TRIGGER T_BLOQUEIO_CANCELAMENTO_ITEM_RESERVA() BEFORE DELETE ON ITEM_RESERVA FOR EACH ROW EXECUTE PROCEDURE T_BLOQUEIO_CANCELAMENTO_ITEM_RESERVA();
