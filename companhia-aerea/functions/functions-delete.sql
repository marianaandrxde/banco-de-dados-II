...
CREATE OR REPLACE FUNCTION REMOVER_ITEM(_ID_ITEM_RESERVA INT) 
RETURNS VOID AS $$
DECLARE ID_ASSENTO INT;
	BEGIN
		PERFORM PRIVATE_VERIFICAR_RESERVA(_ID_RESERVA);

		IF NOT EXISTS (SELECT * FROM ITEM_RESERVA WHERE ID_ITEM_RESERVA = _ID_ITEM_RESERVA) THEN
